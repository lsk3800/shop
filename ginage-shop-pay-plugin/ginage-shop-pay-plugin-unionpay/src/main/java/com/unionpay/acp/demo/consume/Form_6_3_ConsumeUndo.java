...
public class Form_6_3_ConsumeUndo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp)
		...
		data.put("backUrl", UnionpayBase.backUrl);            //后台通知地址，后台通知参数详见open.unionpay.com帮助中心 下载  产品接口规范  网关支付产品接口规范 消费撤销交易 商户通知,其他说明同消费交易的商户通知
		...
		Map<String,String> rspData = AcpService.post(reqData,reqUrl,UnionpayBase.encoding);//发送请求报文并接受同步应答（默认连接超时时间30秒，读取返回结果超时时间30秒）;这里调用signData之后，调用submitUrl之前不能对submitFromData中的键值对做任何修改，如果修改会导致验签不通过
	...
	public static void main(String[] args) {
		...
		data.put("reqType", UnionpayBase.encoding);          //字符集编码 可以使用UTF-8,GBK两种方式
		
		Map<String, String> reqData  = AcpService.sign(data,"d:\\certs\\acp_test_sign_inst.pfx","000000",UnionpayBase.encoding);
		Map<String,String> rspData = AcpService.post(reqData,"http://localhost:8080/QRCSample/notifyReceiveSample",UnionpayBase.encoding);
